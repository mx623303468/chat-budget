...
async function verify(token, secret) {
  ...
  const valid = await crypto.subtle.verify("HMAC", key, signature.buffer, encoder.encode(data));
...
async function signAccessToken(userId, email, secret) {
  return sign({ userId, email }, secret, ACCESS_TOKEN_EXPIRY);
}
__name(signAccessToken, "signAccessToken");
async function signRefreshToken(userId, sessionId, secret) {
...
// ../../node_modules/.pnpm/hono@4.12.14/node_modules/hono/dist/utils/cookie.js
...
// ../../node_modules/.pnpm/hono@4.12.14/node_modules/hono/dist/helper/cookie/index.js
...
__name(setAccessCookie, "setAccessCookie");
...
__name(setRefreshCookie, "setRefreshCookie");
...
auth.post("/register", async (c) => {
  ...
  ).bind(sessionId, userId, refreshTokenHash, now + 7 * 24 * 60 * 60 * 1e3, now, now).run();
...
auth.post("/login", async (c) => {
  ...
  await c.env.DB.prepare(
    "INSERT INTO refresh_sessions (id, user_id, token_hash, expires_at, created_at, last_used_at) VALUES (?, ?, ?, ?, ?, ?)"
  ).bind(sessionId, user.id, refreshTokenHash, now + 7 * 24 * 60 * 60 * 1e3, now, now).run();
...
auth.post("/refresh", async (c) => {
  ...
  ).bind(payload.sessionId, payload.userId, Date.now()).first();
  if (!session) {
...
auth.post("/logout", authMiddleware, async (c) => {
  ...
  if (refreshToken) {
    ...
    if (payload?.sessionId) {
      await c.env.DB.prepare("UPDATE refresh_sessions SET revoked_at = ? WHERE id = ? AND user_id = ?").bind(Date.now(), payload.sessionId, userId).run();
    }
  }
  clearCookies(c);
...
// src/lib/queries.ts
async function getLedgerMembership(db, ledgerId, userId) {
  return db.prepare(
    "SELECT role, removed_at FROM ledger_members WHERE ledger_id = ? AND user_id = ?"
  ).bind(ledgerId, userId).first();
}
__name(getLedgerMembership, "getLedgerMembership");
async function requireMember(db, ledgerId, userId) {
...
async function requireOwner(db, ledgerId, userId) {
...
async function requireLedgerNotDeleted(db, ledgerId) {
  const ledger = await db.prepare("SELECT deleted_at FROM ledgers WHERE id = ?").bind(ledgerId).first();
  if (!ledger || ledger.deleted_at) {
    throw new Error("LEDGER_NOT_FOUND");
  }
  return ledger;
}
__name(requireLedgerNotDeleted, "requireLedgerNotDeleted");

// src/routes/ledgers.ts
var ledgers = new Hono2();
...
ledgers.post("/", async (c) => {
  const userId = c.get("userId");
  const { name, dailyLimit, startDate } = await c.req.json();
  if (!name || !startDate) {
  ...
  await c.env.DB.batch([
    c.env.DB.prepare("INSERT INTO ledgers (id, name, owner_id, daily_limit, start_date, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(ledgerId, name, userId, dailyLimit ?? 0, startDate, now, now),
    c.env.DB.prepare("INSERT INTO ledger_members (ledger_id, user_id, role, joined_at) VALUES (?, ?, ?, ?)").bind(ledgerId, userId, "owner", now),
    c.env.DB.prepare("INSERT INTO ledger_invites (id, ledger_id, code, created_by, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(crypto.randomUUID(), ledgerId, inviteCode, userId, now + 24 * 60 * 60 * 1e3, now)
  ]);
...
ledgers.get("/:id", async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_MEMBER") return c.json({ error: "\u65E0\u6743\u8BBF\u95EE" }, 403);
    throw e;
  }
...
ledgers.put("/:id", async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_MEMBER") return c.json({ error: "\u65E0\u6743\u8BBF\u95EE" }, 403);
    if (e.message === "NOT_OWNER") return c.json({ error: "\u4EC5 owner \u53EF\u4FEE\u6539" }, 403);
    throw e;
  }
  const now = Date.now();
  await c.env.DB.prepare(`
  ...
  `).bind(name ?? null, dailyLimit !== void 0 ? dailyLimit : null, startDate ?? null, now, ledgerId).run();
  const ledger = await c.env.DB.prepare("SELECT * FROM ledgers WHERE id = ?").bind(ledgerId).first();
  return c.json({ ledger });
});
ledgers.delete("/:id", async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_MEMBER") return c.json({ error: "\u65E0\u6743\u8BBF\u95EE" }, 403);
    if (e.message === "NOT_OWNER") return c.json({ error: "\u4EC5 owner \u53EF\u5220\u9664" }, 403);
    throw e;
  }
  const now = Date.now();
...
ledgers.post("/:id/transfer", async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_OWNER") return c.json({ error: "\u4EC5 owner \u53EF\u8F6C\u8BA9" }, 403);
    throw e;
  }
...
var ledgers_default = ledgers;

// src/routes/members.ts
var members = new Hono2();
...
members.delete("/me", async (c) => {
  ...
  await c.env.DB.prepare("UPDATE ledger_members SET removed_at = ? WHERE ledger_id = ? AND user_id = ? AND removed_at IS NULL").bind(Date.now(), ledgerId, userId).run();
  return c.json({ ok: true });
});
members.delete("/:uid", async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_OWNER") return c.json({ error: "\u4EC5 owner \u53EF\u79FB\u9664\u6210\u5458" }, 403);
    throw e;
  }
  if (targetUserId === userId) {
    return c.json({ error: "\u4E0D\u80FD\u79FB\u9664\u81EA\u5DF1\uFF0C\u8BF7\u4F7F\u7528\u8F6C\u8BA9\u529F\u80FD" }, 400);
  }
  await c.env.DB.prepare("UPDATE ledger_members SET removed_at = ? WHERE ledger_id = ? AND user_id = ? AND removed_at IS NULL").bind(Date.now(), ledgerId, targetUserId).run();
  return c.json({ ok: true });
});
var members_default = members;

// src/routes/invites.ts
var invites = new Hono2();
invites.get("/ledgers/:id/invite", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_OWNER") return c.json({ error: "\u4EC5 owner \u53EF\u67E5\u770B\u9080\u8BF7\u7801" }, 403);
    throw e;
  }
...
invites.post("/ledgers/:id/invite/rotate", authMiddleware, async (c) => {
  ...
  try {
    ...
    if (e.message === "LEDGER_NOT_FOUND") return c.json({ error: "\u8D26\u672C\u4E0D\u5B58\u5728" }, 404);
    if (e.message === "NOT_OWNER") return c.json({ error: "\u4EC5 owner \u53EF\u91CD\u65B0\u751F\u6210\u9080\u8BF7\u7801" }, 403);
    throw e;
  }
  const now = Date.now();
  const newCode = generateInviteCode();
  await c.env.DB.prepare("UPDATE ledger_invites SET revoked_at = ? WHERE ledger_id = ? AND revoked_at IS NULL").bind(now, ledgerId).run();
  const inviteId = crypto.randomUUID();
  await c.env.DB.prepare("INSERT INTO ledger_invites (id, ledger_id, code, created_by, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?)").bind(inviteId, ledgerId, newCode, userId, now + 24 * 60 * 60 * 1e3, now).run();
  return c.json({ invite: { id: inviteId, code: newCode, expiresAt: now + 24 * 60 * 60 * 1e3 } });
});
...
invites.post("/invites/join", authMiddleware, async (c) => {
  ...
  const existing = await c.env.DB.prepare("SELECT removed_at FROM ledger_members WHERE ledger_id = ? AND user_id = ?").bind(invite.ledger_id, userId).first();
  if (existing && !existing.removed_at) {
    return c.json({ error: "\u5DF2\u662F\u8BE5\u8D26\u672C\u6210\u5458" }, 409);
  }
  if (existing) {
    await c.env.DB.prepare("UPDATE ledger_members SET role = ?, joined_at = ?, removed_at = NULL WHERE ledger_id = ? AND user_id = ?").bind("member", now, invite.ledger_id, userId).run();
  } else {
    await c.env.DB.prepare("INSERT INTO ledger_members (ledger_id, user_id, role, joined_at) VALUES (?, ?, ?, ?)").bind(invite.ledger_id, userId, "member", now).run();
  }
  return c.json({ ledgerId: invite.ledger_id });
});
var invites_default = invites;
...
function encodeCursor(date, createdAt, id) {
  return btoa(JSON.stringify({ date, createdAt, id }));
}
__name(encodeCursor, "encodeCursor");
function buildTransactionPayloadFromFields(id, ledgerId, userId, amount, note, date, createdAt, updatedAt, deletedAt, createdBy, updatedBy, deletedBy, version) {
...
__name(buildTransactionPayloadFromFields, "buildTransactionPayloadFromFields");
transactions.get("/", async (c) => {
  ...
  if (cData) {
    rows = (await c.env.DB.prepare(`
      ...
      LIMIT ?
    `).bind(ledgerId, cData.date, cData.date, cData.createdAt, cData.date, cData.createdAt, cData.id, limit).all()).results;
  } else {
    rows = (await c.env.DB.prepare(`
      SELECT * FROM transactions
      WHERE ledger_id = ? AND deleted_at IS NULL
      ORDER BY date DESC, created_at DESC, id DESC
      LIMIT ?
    `).bind(ledgerId, limit).all()).results;
  }
  const nextCursor = rows.length === limit && rows.length > 0 ? encodeCursor(rows[rows.length - 1].date, rows[rows.length - 1].created_at, rows[rows.length - 1].id) : null;
...
transactions.post("/", async (c) => {
  ...
  if (!body.id || !body.clientMutationId || body.amount === void 0 || !body.note || !body.date) {
  ...
  ).bind(ledgerId, userId, body.clientMutationId).first();
  ...
  await c.env.DB.batch([
    ...
    ).bind(body.clientMutationId, ledgerId, userId, "create_transaction", "transaction", body.id, "completed", JSON.stringify(responsePayload), now + 30 * 24 * 60 * 60 * 1e3, now)
  ]);
...
transactions.put("/:tid", async (c) => {
  ...
  if (!body.clientMutationId || body.version === void 0) {
  ...
  const tx = await c.env.DB.prepare("SELECT * FROM transactions WHERE id = ? AND ledger_id = ?").bind(transactionId, ledgerId).first();
  ...
  if (tx.created_by !== userId) {
    const membership = await c.env.DB.prepare("SELECT role FROM ledger_members WHERE ledger_id = ? AND user_id = ? AND removed_at IS NULL").bind(ledgerId, userId).first();
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare(
      "INSERT INTO ledger_events (ledger_id, type, entity_type, entity_id, actor_user_id, client_mutation_id, payload, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(ledgerId, "transaction_updated", "transaction", transactionId, userId, body.clientMutationId, buildTransactionPayloadFromFields(transactionId, ledgerId, tx.user_id, newAmount, newNote, newDate, tx.created_at, now, null, tx.created_by, userId, null, newVersion), now),
    c.env.DB.prepare(
      "INSERT INTO client_mutations (id, ledger_id, user_id, operation_type, entity_type, entity_id, status, response_payload, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(body.clientMutationId, ledgerId, userId, "update_transaction", "transaction", transactionId, "completed", JSON.stringify(responsePayload), now + 30 * 24 * 60 * 60 * 1e3, now)
  ]);
...
transactions.delete("/:tid", async (c) => {
  ...
  const tx = await c.env.DB.prepare("SELECT * FROM transactions WHERE id = ? AND ledger_id = ? AND deleted_at IS NULL").bind(transactionId, ledgerId).first();
  if (!tx) {
    return c.json({ ok: true });
  }
  if (tx.created_by !== userId) {
  ...
  if (clientMutationId) {
    ...
    ).bind(ledgerId, userId, clientMutationId).first();
    if (existing && existing.status === "completed") {
      return c.json({ ok: true });
    }
  }
  ...
  const batchStmts = [
    c.env.DB.prepare("UPDATE transactions SET deleted_at = ?, deleted_by = ?, updated_at = ?, version = ? WHERE id = ?").bind(now, userId, now, newVersion, transactionId),
    c.env.DB.prepare(
      "INSERT INTO ledger_events (ledger_id, type, entity_type, entity_id, actor_user_id, client_mutation_id, payload, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  ...
  if (clientMutationId) {
    batchStmts.push(
      c.env.DB.prepare(
        "INSERT INTO client_mutations (id, ledger_id, user_id, operation_type, entity_type, entity_id, status, response_payload, expires_at, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
      ).bind(clientMutationId, ledgerId, userId, "delete_transaction", "transaction", transactionId, "completed", JSON.stringify({ ok: true }), now + 30 * 24 * 60 * 60 * 1e3, now)
    );
  }
...
var SyncDO = class {
  ...
  getOnlineMembers() {
    return Array.from(this.clients.values()).filter((c) => c.subscribed).map((c) => ({ userId: c.userId, nickname: c.nickname }));
  }
  broadcastEvent(event, excludeUserId) {
...
app.route("/api/ledgers", ledgers_default);
app.route("/api/ledgers/:id/members", members_default);
app.route("/api/ledgers/:id/transactions", transactions_default);
app.route("/api/ledgers/:id/events", events_default);
app.route("/api", invites_default);
app.get("/api/ws/:ledgerId", async (c) => {
  ...
  const user = await c.env.DB.prepare("SELECT nickname FROM users WHERE id = ?").bind(userId).first();
...
// .wrangler/tmp/bundle-X49K47/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-X49K47/middleware-loader.entry.ts
